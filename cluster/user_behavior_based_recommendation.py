...
def user_beavior_based_recommendation(user_lst, user_behav_distri, user_buy_ratio, item_buy_ratio, user_behav_last_days, end_time, min_possi_ratio, min_buy_ratio):
    recommendation_dict = {}
    for ti in user_behav_last_days:
        if ti[0] not in user_behav_distri or ti[2] == '4' or ti[0] not in user_lst:
            continue
...
def recommendation(max_day_interv, min_possi_ratio, min_buy_ratio, end_time, flush=False, isTest=True):
    ...
    user_lst = dbHandler.read_table(TARGET_USER_TABLE)#(TARGET_USER_TABLE)
    ...
    recommendation_dict = user_beavior_based_recommendation(user_lst, user_behav_distri, user_buy_ratio, item_buy_ratio, user_behav_last_days, end_time, min_possi_ratio, min_buy_ratio)
    return recommendation_dict
...
def init(flush=False, isTest=True, end_time=TEST_END_TIME):
    ...
    if not dbHandler.table_exist(TARGET_USER_TABLE):
        ...
        q = 'select distinct user_id, count(*) from %s where behavior_type=4 group by user_id having count(*)>=2' % (TRAIN_DELETE_NOISY_DATA)
        cursor.execute(q)
...
if __name__ == '__main__':
    ...
    reccomendation_dict = recommendation(max_day_interv, min_possi_ratio, min_buy_ratio, end_time, flush, isTest)

# for running test, isTest set to True, for submit, isTest set to False